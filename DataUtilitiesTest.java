...
//14
 ...
 public void testInvalidRowIndex() {
     ...
     public void testNormalBoundaryRowTotal() {
         // Create a valid data table
         DefaultKeyedValues2D testObject = new DefaultKeyedValues2D();
         testObject.addValue(3, 0, 1); // Adding value 3 at row index 0 and column index 1
         
         // Test normal boundary row index (0)
         assertEquals("Incorrect output", 3, DataUtilities.calculateRowTotal(testObject, 0), 0.0000001d);
     }
     //4
     @Test(expected = IndexOutOfBoundsException.class)
  ...
  // 11
     ...
     public void testCreateNumberArray_ValidInput_PositiveValues() {
         ...
         public void testGetCumulativePercentages() {
             ...
             ((DefaultKeyedValues) data).addValue("Key1", 10.0);
             result = DataUtilities.getCumulativePercentages(data);
             assertEquals(1, result.getItemCount());
             assertEquals(0.9, result.getValue(0).doubleValue(), 0.0001);
             
             // Test case with multiple values
             ((DefaultKeyedValues) data).addValue("Key2", 20.0);
             ((DefaultKeyedValues) data).addValue("Key3", 30.0);
             result = DataUtilities.getCumulativePercentages(data);
             assertEquals(3, result.getItemCount());
             assertEquals(0.15, result.getValue(0).doubleValue(), 0.0001);
             assertEquals(0.45, result.getValue(1).doubleValue(), 0.0001);
             assertEquals(0.9, result.getValue(2).doubleValue(), 0.0001);
             
             // Test case with null value
             ((DefaultKeyedValues) data).addValue("Key4", null);
             result = DataUtilities.getCumulativePercentages(data);
             assertEquals(4, result.getItemCount());
             assertEquals(0.15, result.getValue(0).doubleValue(), 0.0001);
             assertEquals(0.45, result.getValue(1).doubleValue(), 0.0001);
             assertEquals(0.9, result.getValue(2).doubleValue(), 0.0001);
             assertEquals(0.9, result.getValue(3).doubleValue(), 0.0001);
             
             // Test case with all null values
             data = new DefaultKeyedValues();
             ((DefaultKeyedValues) data).addValue("Key1", null);
             ((DefaultKeyedValues) data).addValue("Key2", null);
             result = DataUtilities.getCumulativePercentages(data);
             assertEquals(2, result.getItemCount());
             assertEquals(Double.NaN, result.getValue(0).doubleValue(), 0.0001);
             assertEquals(Double.NaN, result.getValue(1).doubleValue(), 0.0001);
             
             // Test case with a mix of null and non-null values
             data = new DefaultKeyedValues();
             ((DefaultKeyedValues) data).addValue("Key1", 10.0);
             ((DefaultKeyedValues) data).addValue("Key2", null);
             ((DefaultKeyedValues) data).addValue("Key3", 20.0);
             result = DataUtilities.getCumulativePercentages(data);
             assertEquals(3, result.getItemCount());
             assertEquals(0.3, result.getValue(0).doubleValue(), 0.0001);
             assertEquals(0.3, result.getValue(1).doubleValue(), 0.0001);
             assertEquals(0.9, result.getValue(2).doubleValue(), 0.0001);
             
             // Test case with zero total
             data = new DefaultKeyedValues();
             ((DefaultKeyedValues) data).addValue("Key1", 0.0);
             ((DefaultKeyedValues) data).addValue("Key2", 0.0);
             result = DataUtilities.getCumulativePercentages(data);
             assertEquals(2, result.getItemCount());
             assertEquals(Double.NaN, result.getValue(0).doubleValue(), 0.0001);
             assertEquals(Double.NaN, result.getValue(1).doubleValue(), 0.0001);
